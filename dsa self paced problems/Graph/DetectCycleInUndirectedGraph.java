...
public class DetectCycleInUndirectedGraph {
    ...
    public static boolean detectCycle(ArrayList<ArrayList<Integer>> adj, int V) {
        ...
        for (int i = 0; i < V; i++) {
            if (!found[i] && dfs(adj, found, i, -1))
                return true; // as it's the first vertext of the disconnected part of the graph
        }
        return false;
    }

    public static boolean dfs(ArrayList<ArrayList<Integer>> adj, boolean found[], int vertex,
            int parent) {
        found[vertex] = true;
        for (int x : adj.get(vertex)) {
            if (found[x] && x != parent) // present into the found but does not equal with the parent
                return true;
            if (!found[x] && dfs(adj, found, x, vertex)) //have not found yet, so we check for it's child
                return true;
        }
        return false;
    }
